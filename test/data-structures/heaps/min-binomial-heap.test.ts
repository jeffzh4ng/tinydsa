import MinBinomialHeap from '../../../src/data-structures/priority-queues/mergeable-heaps/min-binomial-heap'

describe('MinBinomialHeap', () => {
  let heap: MinBinomialHeap<number>

  beforeEach(() => {
    heap = new MinBinomialHeap(Number.MIN_SAFE_INTEGER)
  })
  ...
  describe('Updating', () => {
    it('unions properly', () => {
      const heapA = new MinBinomialHeap(Number.MIN_SAFE_INTEGER)
      const heapB = new MinBinomialHeap(Number.MIN_SAFE_INTEGER)
      ...
      const newHeap = heapA.union(heapB)
      expect(newHeap.size).toBe(heapA.size + heapB.size)

      const sortedValues = [...valuesA, ...valuesB].sort((a, b) => a - b)

      for (const v of sortedValues) {
        const node = newHeap.dequeue()
        if (!node) throw new Error()
        expect(node.value).toBe(v)
      }
    })
    ...
    it('decreases keys properly', () => {
      const node = heap.enqueue(3)

      expect(heap.decreaseKey(node, 5)).toBe(false)
